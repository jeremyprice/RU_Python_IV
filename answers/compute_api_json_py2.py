...
def get_new_auth_response_api_key(user, api_key):
    """ Gets the auth response using a username:api_key. """
    auth_json_creds = '{ "auth":{ "RAX-KSKEY:apiKeyCredentials":{ "username":"%s", "apiKey":"%s"}}}' % (user, api_key)
    return get_new_auth_response(auth_json_creds)

def get_new_auth_response_passwd(user, passwd):
    """ Gets the auth response using a username:passwd. """
    auth_json_creds = '{ "auth":{ "passwordCredentials":{ "username":"%s","password":"%s"}}}' % (user, passwd)
    return get_new_auth_response(auth_json_creds)

def get_new_auth_response(auth_json_creds):
